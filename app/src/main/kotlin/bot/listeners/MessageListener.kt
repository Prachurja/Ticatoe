	...
	override fun onGuildMessageReceived(event: GuildMessageReceivedEvent) {
		...
		if(messageText.startsWith("!tictactoe")) {
	    	...
	    	when(mentioned.size) {
	    		0 -> {
	    			event.getMessage()
	     			.reply(errorEmbed.generate("You need to mention the player to play against!"))
	    			.queue()
	    		}

	    		1 -> {
	    			if(!commandUsers.containsKey(event.getMember()!!)) {
	    				commandUsers.put(event.getMember()!!, LocalTime.now().minusMinutes(1))
	    			}

	    			val sinceLastCMD: Duration = Duration.between(commandUsers.get(event.getMember()!!), LocalTime.now())
	    		    ...
	    		    if(players.toList().toSet().size == 1) {
	    		    	event.getMessage()
	    	    	    .reply(errorEmbed.generate("You can't play against yourself."))
	    	    	    .queue()
	    		    }

	    		    else if(players.toList().filter{it.getUser().isBot()}.size > 0) {
	    		    	event.getMessage()
	    	    	    .reply(errorEmbed.generate("You can't play against a bot."))
	    	    	    .queue()
	    		    }
	        	        ...
	        	        commandUsers.put(event.getMember()!!, LocalTime.now())
		            }
		            	...
		            	event.getMessage()
	   	            	.getChannel()
	   	            	.sendMessage(tooFastEmbed.generate("There's a $secondsToWait seconds cooldown. You have to wait for $secondsToWaitMore seconds before you can use this command again."))
	        	        .queue()
		            }
	    		}
	    			...
	    			event.getMessage()
	    		    .reply(errorEmbed.generate("You can't play tic-tac-toe with multiple players, can you?"))
	    		    .queue()
	    		}
	    	}
		}
			...
			if(pair != null) {
				if(messageText.equals(Responses.REACT.generate(pair))) {
	       			...
	       			requests.put(event.getMessage().getId(), request)
	       		}
