...
class ReactionListener private constructor(): ListenerAdapter()
{
	...
	private fun reaction(event: GenericMessageReactionEvent)
	{
		...
		val request: GameRequest? = RequestsData.requests.get(event.getMessageId())
		val game: Game? = RequestsData.runningGames.get(event.getMessageId())
	    		...
	    		if(game.running())
	    		{
	    			...
	    			if(game.currentMember == event.getMember())
	    			{
	            		...
	            		if(direction != null)
	            		{
	       	    		...
	       	    		{
	       	    			game.click()
	       	    			game.render(color = EmbedData.successEC)
	   	        		}
