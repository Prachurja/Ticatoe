...
class Game(val message: Message, val members: Pair<Member, Member>): Thread()
{
	...
	override fun run()
	{
		RequestsData.runningGames.put(message.getId(), this)
		...
		if(draw())
		{
			render("${members.first.getAsMention()} and ${members.second.getAsMention()} are tied.", EmbedData.tieEC, showCursor = false, runningSpecial = true)
		}

		else if(!running())
		{
			render("${currentMember.getAsMention()} has won the game. Congrats! :tada:", EmbedData.winEC, showCursor = false, runningSpecial = true)
		}
		
		RequestsData.runningGames.remove(message.getId())
	}
	...
	fun render(comment: String = "${currentMember.getAsMention()}'s turn", color: EmbedColor = EmbedData.normalEC, showCursor: Boolean = true, runningSpecial: Boolean = false)
	{
